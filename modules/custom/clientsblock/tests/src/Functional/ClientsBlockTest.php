...
namespace Drupal\Tests\clientsblock\Functional;
 ...
 * @group examples
 */
class ClientsBlockTest extends BrowserTestBase {
   ...
   * @var array
   */
  public static $modules = array('block', 'clientsblock');

  /**
   * Tests block_example functionality.
   */
  public function testClientsBlockBasic() {
    ...
    $settings_configurable = array(
      ...
      'id' => 'clientsblock_example_configurable_text',
      'theme' => $theme_name,
    );
    ...
    $settings_uppercase = array(
      ...
      'id' => 'clientsblock_example_uppercased',
      'theme' => $theme_name,
    );
    ...
    $edit = array(
      'settings[clientsblock_string_text]' => $this->randomMachineName(),
    );
    ...
    $this->drupalGet('');
    $assert->statusCodeEquals(200);
    $assert->pageTextContains($edit['settings[clientsblock_string_text]']);
  }
